/**
 * CreateOutfitContent - Reusable component for creating/editing outfits
 *
 * This component contains all the logic from outfit/create.tsx but accepts
 * an optional returnRoute prop for custom navigation behavior.
 *
 * Used by:
 * - app/outfit/create.tsx (standalone route)
...
export interface CreateOutfitContentProps {
  // Route params
  outfitId?: string;
  // Custom navigation
  returnRoute?: string;
...
/**
 * CreateOutfitContent - Two-step outfit creation process
 * Step 1: Select items from wardrobe by category
 * Step 2: Compose items on canvas with drag & drop
 */
export function CreateOutfitContent({ outfitId, returnRoute }: CreateOutfitContentProps) {
  ...
  useEffect(() => {
    const loadWardrobeItems = async () => {
      ...
      try {
        console.log('📦 [CreateOutfitContent] Loading wardrobe items from DB...');
        const items = await itemServiceOffline.getUserItems(user.id);
        console.log(`✅ [CreateOutfitContent] Loaded ${items.length} wardrobe items`);

        const { setItems } = useWardrobeStore.getState();
        setItems(items);
      } catch (error) {
        console.error('❌ [CreateOutfitContent] Failed to load wardrobe items:', error);
      }
    };
  ...
  const loadOutfitForEdit = async (id: string) => {
    try {
      ...
      const outfit = await outfitServiceOffline.getOutfitById(id);
  ...
  const confirmSave = useCallback(async () => {
    ...
    try {
      ...
      if (isEditMode && outfitId) {
        await outfitServiceOffline.updateOutfit(outfitId, {
